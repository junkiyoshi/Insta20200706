	...
	for (int x = -200; x <= 200; x += span) {
		...
		for (int y = -200; y <= 200; y += span) {
			...
			for (int z = -200; z <= 200; z += span) {
				...
				auto param = (int)(ofRandom(1000) + ofGetFrameNum() * 0.5) % (span * 4);

				
				if (param < span) {

					auto size = ofMap(param, 0, span, 0, span);
					this->setBoxToMesh(this->face, this->frame, glm::vec3(x, y, z), size, ofColor(39), ofColor(239));
				}
				else if(param < span * 2) {

					this->setBoxToMesh(this->face, this->frame, glm::vec3(x, y, z), span, ofColor(39), ofColor(239));
				}
				else if(param < span * 3) {

					this->setBoxToMesh(this->face, this->frame, glm::vec3(x, y, z), span, ofColor(ofMap(param, span * 2, span * 3, 39, 239)), ofColor(ofMap(param, span * 2, span * 3, 239, 39)));
				}
			}
		}
	}
